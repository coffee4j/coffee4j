...
public abstract class ModelBasedConstraintChecker implements ConstraintChecker {
    ...
    private void addAssignmentConstraint(int parameter, int value, Model model, List<Constraint> constraints) {
        ...
        final IntVar variable = findVariable(model, parameter)
                .filter(v -> v instanceof IntVar)
                .map(v -> (IntVar) v)
                .orElseThrow(() -> new IllegalStateException("unknown variable"));
    ...
    public void addConstraint(int[] forbiddenTuple) {
        // no action needed
        // if a subclass allows for adding constraint, this method can be overwritten
    }
