...
public class ReduceBasedDiagnosisHittingSetBuilder {
    ...
    public List<DiagnosisHittingSet> computeMinimalDiagnosisHittingSets(List<MissingInvalidTuple> missingInvalidTuples) {
        ...
        Preconditions.check(missingInvalidTuples.stream().allMatch(mit -> isDiagnosisSetsOrInconsistentBackground(mit.getExplanation())));

        final List<MissingInvalidTuple> extendedMissingInvalidTuples
                = extendWithDiagnosisSetsForNegatedErrorConstraints(missingInvalidTuples);

        final List<Set<Set<DiagnosisElement>>> sets = extendedMissingInvalidTuples.stream()
    ...
    boolean isMinimalDiagnosisHittingSet(Set<DiagnosisElement> candidate, Set<Set<DiagnosisElement>> diagnosisHittingSets) {
        ...
        for(Set<DiagnosisElement> other : diagnosisHittingSets) {
            if(candidate != other && coversAnotherDiagnosisHittingSet(candidate, other)) {
                return false;
            }
        }
    ...
    private List<MissingInvalidTuple> extendWithDiagnosisSetsForNegatedErrorConstraints(
            List<MissingInvalidTuple> missingInvalidTuples) {
    ...
    private TupleList findConstraint(int constraintId) {
        return testModel.getExclusionTupleLists().stream()
                .filter(tupleList -> tupleList.getId() == constraintId)
                .findFirst()
                .or(() -> testModel.getErrorTupleLists().stream()
                        .filter(tupleList -> tupleList.getId() == constraintId)
                        .findFirst())
                .orElseThrow(() -> new IllegalStateException("unknown constraint with id " + constraintId));
    }
}
