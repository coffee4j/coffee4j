...
final class CombinationPartitioner {
     ...
     *                    parameter
     * @throws NullPointerException     if combination is {@code null}
     * @tpublichrows IllegalArgumentException if the combination does not contain the
     *                                  partitioning parameter
     */
     ...
     * @param constraintChecker checker that is used for extending the existing combination without creating invalid
     *                          combinations.
    ...
    Optional<int[]> extendSuitableCombination(int[] combination, ConstraintChecker constraintChecker) {
        Preconditions.notNull(combination, COMBINATION_NOT_NULL);
        Preconditions.check(combination.length > partitioningParameter, PARAMETER_NOT_IN_COMBINATION);
        final List<IntArrayWrapper> possibleCombinations = new LinkedList<>(partitioner.get(combination[partitioningParameter]));
