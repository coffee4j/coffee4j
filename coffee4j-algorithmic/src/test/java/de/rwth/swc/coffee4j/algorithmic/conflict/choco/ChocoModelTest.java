...
import de.rwth.swc.coffee4j.algorithmic.constraint.NegatedConstraint;
...
public class ChocoModelTest {
    ...
    void testIntegration() {
        ...
        final CompleteTestModel combinatorialTestModel = CompleteTestModel.builder()
        ...
        constraints.addAll(converter.convertAll(combinatorialTestModel.getExclusionTupleLists()));
    ...
    public static ChocoModel createTestModel(int idToBeNegated) {
        ...
        add(idToBeNegated, constraints,
                new Constraint(tupleList(4), (Model model)
                        -> model.or(
                                model.and(model.arithm((IntVar) findVariable(model, 0).orElseThrow(IllegalStateException::new), "=", 0), model.arithm((IntVar) findVariable(model, 1).orElseThrow(IllegalStateException::new), "=", 1)),
                                model.and(model.arithm((IntVar) findVariable(model, 0).orElseThrow(IllegalStateException::new), "=", 0), model.arithm((IntVar) findVariable(model, 1).orElseThrow(IllegalStateException::new), "=", 2))
                        ).getOpposite())
        );
        add(idToBeNegated, constraints,
                new Constraint(tupleList(5), (Model model) ->
                        model.or(
                                model.and(model.arithm((IntVar) findVariable(model, 0).orElseThrow(IllegalStateException::new), "=", 1), model.arithm((IntVar) findVariable(model, 1).orElseThrow(IllegalStateException::new), "=", 0)),
                                model.and(model.arithm((IntVar) findVariable(model, 0).orElseThrow(IllegalStateException::new), "=", 1), model.arithm((IntVar) findVariable(model, 1).orElseThrow(IllegalStateException::new), "=", 2))
                        ).getOpposite())
        );
