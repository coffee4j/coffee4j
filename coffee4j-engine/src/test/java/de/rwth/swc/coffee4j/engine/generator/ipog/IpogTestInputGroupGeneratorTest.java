...
class IpogTestInputGroupGeneratorTest {
    ...
    void simpleGenerationTest() {
        final CombinatorialTestModel model = new CombinatorialTestModel(1, new int[]{2}, Collections.emptyList(), Collections.emptyList());
        
        final Set<Supplier<TestInputGroup>> group = new IpogTestInputGroupGenerator().generate(model, Mockito.mock(Reporter.class));
        
        assertEquals(1, group.size());
        final TestInputGroup generatedGroup = new ArrayList<>(group).get(0).get();
        assertEquals(2, generatedGroup.getTestInputs().size());
        assertEquals(model, generatedGroup.getFaultCharacterizationConfiguration().get().getModel());
