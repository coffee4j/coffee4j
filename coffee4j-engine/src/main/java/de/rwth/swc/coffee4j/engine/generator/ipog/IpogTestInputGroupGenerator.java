 ...
 * {@link Ipog} algorithm using no constraints and the normal parameter order
 * with the strength given by the {@link CombinatorialTestModel}.
 */
public class IpogTestInputGroupGenerator implements TestInputGroupGenerator {
    
    private static final String DISPLAY_NAME = "Positive Ipog Tests";
    ...
    public Set<Supplier<TestInputGroup>> generate(CombinatorialTestModel model, Reporter reporter) {
        return Collections.singleton(() -> {
            ...
            final List<int[]> testInputs = new Ipog(IpogConfiguration.ipogConfiguration().model(model).checker(factory.createHardConstraintsChecker()).build()).generate();
